function [tstat, pval] = goftest_a4(u,A,family,theta,varargin)
% Conducts a goodness-of-fit test of type A4 (Berg(2009)) for  a specified 
% simplified vine copula structure and given data. The test is based on a 
% Cramer-von Mises statistic of the difference between the  Kendall 
% distribution functions of the empirical copula and of the estimated 
% copula.
% 
...
% previous step, see also function ssp). Under the null hypothesis the 
% estimated copula model has appropriate fit.
% 
...
for ii = 1:1:size(family,1)
    ...
    for jj = 1:1:size(family,2)-ii+1
        ...
        if ~cpcheck(family{ii,jj},theta{ii,jj})
            error('goftest_a4:InvalidParameter',['invalid parameter for ',family{ii,jj},' copula at (',num2str(ii),',',num2str(jj),')']);    
        end
...
if (size(A,1) ~= size(A,2))
    error('goftest_a4:InvalidVineArray','vine array A has to be a quadratic matrix');
end

for jj = 1:1:size(A,1)
    
    if (length(unique(A(1:jj,jj))) ~= jj)
        error('goftest_a4:InvalidVineArray','input A is not a vine array');
    end
...
if nargin > 4
    if varargin{1} ~= 1 && varargin{1} ~= 0
        error('goftest_a4:InvalidParallelizationMode','optional input argument parallel has to be 0 or 1')
    end
    para = varargin{1};
end
...
% 2 - analytical expression unknown, therefore approximate Kendall
%     distribution function
x_star = simrvine(n_b,A,family,theta);
c_hatstar = ecopula(x_star);

s_hat = sum(repmat(c_hat',n_b,1)<=repmat(c_hatstar,1,n),2)/(n_b-1);
s_hatstar = sum(repmat(c_hatstar',n_b,1)<=repmat(c_hatstar,1,n_b),2)/(n_b-1);

% 3 - Cramer-von Mises test statistic
tstat = n/n_b*sum((s_hat-s_hatstar).^2);
...
if para
    ...
    parfor (ii=1:k,poolsize)
        ...
        % approximate Kendall distribution function
        x_starbs = simrvine(n_b,A,family,thetabs);
        c_thetahatbs = ecopula(x_starbs);

        s_hatbs = sum(repmat(c_hatbs',n_b,1)<=repmat(c_thetahatbs,1,n),2)/(n_b-1);
        s_hatstarbs = sum(repmat(c_thetahatbs',n_b,1)<=repmat(c_thetahatbs,1,n_b),2)/(n_b-1);

        % save bootstrapped test statistic
        tstatbs(ii) = n/n_b*sum((s_hatbs-s_hatstarbs).^2);
